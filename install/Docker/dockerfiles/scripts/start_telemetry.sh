...
start_telemetry()
{
	...
	nohup /bin/svc-telemetry --registry=etcd --framework=GRPC --registry_address=${registry_address} --server_address=telemetry:45111 --client_request_timeout=`expr $(cat $CONFIG_FILE_PATH | grep SouthBoundRequestTimeoutInSecs | cut -d : -f2 | cut -d , -f1 | tr -d " ")`s >> /var/log/odimra_logs/telemetry.log 2>&1 &
	PID=$!
	sleep 3
