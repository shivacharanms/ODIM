...
func Publish(data interface{}) bool {
	...
	if strings.Contains(metricReportEventDataMap["@odata.type"].(string), "MetricReport") {
		// prepare the device data
		var devices []rfpmodel.Device
		rfpmodel.GetAllDevicesInInventory(&devices)

		systemUUID := getMatchingDeviceUUID(event.IP, devices)
		if systemUUID == "" {
			return false
		}

		// to replace id in system
		updatedData := strings.Replace(string(event.Request), "/redfish/v1/Systems/", "/redfish/v1/Systems/"+systemUUID+":", -1)
